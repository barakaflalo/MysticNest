...
function deckText(he,en){return state.lang==='he'?he:(UI_LOCALES[state.lang]?.[en]??en);}
function drawTarot(){
...
function renderTarotDraw(){
  ...
  const positions=spreadPositions(sp);
  ...
  hint.textContent=deckText(U('לחיצה לחשיפה · לחיצה נוספת להגדלה'),'Tap to reveal · tap again to enlarge');
  ...
  currentDraw.picks.forEach((pick,i)=>{
    ...
    button.setAttribute('aria-label',positions[i]+' — '+deckText(U('חשוף קלף'),'Reveal card'));
    ...
    button.querySelector('.card-name').textContent=deckText(U('קלף נסתר'),'Hidden card');
...
function showMissingArt(container,card){
  ...
  const title=document.createElement('strong');title.textContent=cardName(card);
  const msg=document.createElement('span');msg.textContent=MYSTIC_DECK.availableIds&&!MYSTIC_DECK.availableIds.includes(card.id)?deckText(U('האיור בהכנה. הפירוש זמין למטה.'),'Artwork in progress. The meaning is available below.'):deckText(U('האיור אינו זמין כרגע. הפירוש זמין למטה.'),'Artwork unavailable. The meaning is available below.');
...
function flipCard(i){
  ...
  button.querySelector('.card-name').textContent=state.lang==='he'?cardName(pick.card):pick.card.en;
  const orientation=pick.rev?deckText(U('הפוך'),'Reversed'):deckText(U('ישר'),'Upright');
  button.querySelector('.card-state').textContent=orientation;
  button.setAttribute('aria-label',cardName(pick.card)+' · '+orientation+' — '+deckText(U('הגדל קלף'),'Enlarge card'));
...
function showReading(){
  ...
  const sp=SPREADS[currentDraw.spread],positions=spreadPositions(sp);
  ...
  currentDraw.picks.forEach((pick,i)=>{
    ...
    const title=document.createElement('span');title.textContent=cardName(pick.card);
    const position=document.createElement('span');position.className='p';position.textContent=positions[i]+' · '+(pick.rev?deckText(U('הפוך'),'Reversed'):deckText(U('ישר'),'Upright'));
    head.append(thumb,title,position);
    const meaning=document.createElement('div');meaning.className='m';meaning.dir=contentDir();meaning.lang=contentLang();meaning.textContent=cardShort(pick);
...
function openCardDetail(pick,trigger,position){
  ...
  const close=document.createElement('button');close.className='zoom-close';close.textContent=deckText(U('סגור ×'),'Close ×');close.onclick=()=>dialog.close();
  const layout=document.createElement('div');layout.className='zoom-layout';
  const imageWrap=document.createElement('div'),img=document.createElement('img');img.className='zoom-art'+(pick.rev?' rev-art':'');img.alt=cardName(pick.card);img.src=cardImagePath(pick.card.id);img.onerror=()=>showMissingArt(imageWrap,pick.card);imageWrap.append(img);
  const copy=document.createElement('div');copy.className='zoom-copy';
  const title=document.createElement('h2');title.id='zoomTitle';title.textContent=cardName(pick.card);
  const english=document.createElement('p');english.className='en';english.textContent=contentLang()==='he'?pick.card.en:'';
  const orientation=document.createElement('p');orientation.textContent=pick.rev?deckText(U('קלף הפוך'),'Reversed card'):deckText(U('קלף ישר'),'Upright card');
  const meaning=document.createElement('p');meaning.dir=contentDir();meaning.lang=contentLang();meaning.textContent=cardShort(pick);
...
function appendCardDetails(copy,pick){
  const detail=cardDetails(pick.card.id);if(!detail)return;
  const content=document.createElement('div');content.className='card-details';content.dir=contentDir();content.lang=contentLang();
  for(const [heading,text] of [[U('מבט מעמיק'),pick.rev?detail.rev:detail.up],[U('השפה הסמלית של הקלף'),detail.symbols],[U('שאלה למחשבה'),detail.question],[U('צעד קטן שאפשר לנסות'),detail.step]]){
  ...
  const note=document.createElement('p');note.className='detail-note';note.textContent=U('הפירוש מציע כיוון להתבוננות. אפשר לקחת ממנו את מה שמתאים לחוויה שלך.');content.append(note);copy.append(content);
}
