...
function dreamNormalize(t){return String(t||'').normalize('NFKC').replace(/[\u0591-\u05C7]/g,'').toLowerCase().replace(/[^א-תa-z0-9]+/g,' ').trim().split(/\s+/).map(w=>DREAM_SPELLINGS[w]||w).join(' ');}
function dreamFind(text,keys){
 const clauses=String(text).split(/[.!?;\n]+|\s+(?:אבל|אלא)\s+/),found=[];let offset=0;
 ...
 clauses.forEach((clause,ci)=>{const words=dreamNormalize(clause).split(' ').filter(Boolean);
  for(const key of keys){const parts=dreamNormalize(key).split(' ');if(!parts[0])continue;
   for(let i=0;i<=words.length-parts.length;i++){
    ...
    const prev=words.slice(Math.max(0,i-3),i),inherent=['לא','אין','בלי','ללא'].includes(parts[0]);
    const neg=prev.findLastIndex(w=>['לא','ולא','בלי','ללא','אין','ואין'].includes(w));
    if(!inherent&&neg>=0&&prev.slice(neg+1).every(w=>['היה','הייתה','היו','ראיתי','ראינו','מצאתי','פגשתי','שם','שום','לי'].includes(w)))continue;
...
function analyzeLocalDream(txt,emotion='',ending='',excluded=[],personal={}){
 ...
 let summary=found.length?'בחלום זוהו: '+found.slice(0,4).map(r=>r.name).join(' · ')+'. ':'לא זוהו מספיק פרטים מוכרים. אפשר להוסיף תיאור של המקום, הפעולה או הדמות שהיו משמעותיים לך. ';
 if(relevant.length)summary+=relevant[0].meaning+' ';
 else if(chosen)summary+=chosen.meaning+' ';
 const em={fear:'בחרת בפחד: אפשר להתמקד במה שהעניק או היה יכול להעניק ביטחון.',calm:'בחרת ברוגע: אין צורך להעניק לפרטים פירוש מאיים אם החוויה הייתה שלווה.',joy:'בחרת בשמחה: כדאי לזהות מה היה נעים ומה היית רוצה לקחת מן החוויה.',confusion:'בחרת בבלבול: אפשר לברר פרט אחד ולהשאיר את השאר פתוח.',longing:'בחרת בגעגוע: אפשר לתת מקום לזיכרון ולמה שחסר לך.',sadness:'בחרת בעצב: אפשר לברר מה היה יכול לנחם אותך בתוך הסיפור.'};
 if(em[emotion])summary+=em[emotion]+' ';
 const end={resolved:'הסיום שבחרת מפנה תשומת לב גם למה שעזר בסיפור.',unresolved:'הסיום נשאר פתוח; אפשר לדמיין המשך שהיה נותן מענה לצורך שלך.',woke:'התעוררת באמצע; אין צורך להסיק כיצד הסיפור היה מסתיים.'};if(end[ending])summary+=end[ending]+' ';
 let question=relevant[0]?.question||chosen?.question||'איזה פרט נשאר איתך ומה הוא מזכיר לך?';
 if(personal.significance&&chosen){const lens={safe:['הקישור האישי שלך ל־'+chosen.name+' הוא של ביטחון או קרבה. כדאי לתת לכך מקום גם אם דימוי כללי מציע כיוון אחר.','מה בפרט הזה מעניק לך ביטחון?'],uncomfortable:['קישרת את '+chosen.name+' לאי־נוחות. אפשר לברר את הגבול או הצורך שלך בלי להניח שהסמל מאיים כשלעצמו.','איזה שינוי במפגש היה הופך אותו לנוח יותר?'],memory:['קישרת את '+chosen.name+' לזיכרון אישי. אפשר להשוות בין הרגש בזיכרון לבין מה שהיה בחלום.','מה דומה ומה שונה בין הזיכרון לבין החלום?'],new:['קישרת את '+chosen.name+' לדבר חדש או מסקרן. אפשר להשאיר מקום לחקירה בלי למהר לקבוע משמעות.','מה היית רוצה לשאול או לגלות על הפרט הזה?']};if(lens[personal.significance]){summary+=lens[personal.significance][0];question=lens[personal.significance][1];}}
 if(personal.note?.trim())question='בהתייחס למה שכתבת על הקשר האישי: '+question;
...
function setupDreamExpansion(){
 ...
 const more=dreamEl('details',null,'dream-personal');more.append(dreamEl('summary','הקשר האישי שלי (אופציונלי)'));
 for(const [id,label,options] of [['dreamFocus','איזה פרט מרכזי עבורך?',{'':'אפשר לבחור אחרי הפירוש'}],['dreamSignificance','למה הפרט מתקשר אצלך?',{'':'ללא בחירה',safe:'ביטחון או קרבה',uncomfortable:'אי־נוחות או פחד',memory:'זיכרון אישי',new:'משהו חדש או מסקרן'}]]){const l=dreamEl('label',label,'lbl');l.htmlFor=id;const select=dreamEl('select',null,'field');select.id=id;for(const [v,t] of Object.entries(options))select.append(new Option(t,v));more.append(l,select);}
 const label=dreamEl('label','מה זה מזכיר לך? (במילים שלך)','lbl');label.htmlFor='dreamPersonal';const note=dreamEl('textarea',null,'field');note.id='dreamPersonal';note.maxLength=1500;more.append(label,note,dreamEl('p','הבחירות מכוונות את הקריאה המקומית. הטקסט החופשי נשמר כמחשבה שלך; הבינה יכולה להיעזר בו רק כשתבחר בהעמקה.','small'));controls.append(more);
 const catalog=dreamCatalog(),wrap=dreamEl('details',null,'reading dream-library');wrap.append(dreamEl('summary','עיון במאגר החלומות'),dreamEl('p',catalog.length+' סמלים ומצבים · '+DREAM_EXPANSION.combinations.length+' חיבורים אפשריים','small'));
 const search=dreamEl('input',null,'field');search.type='search';search.setAttribute('aria-label','חיפוש במאגר החלומות');search.placeholder='חיפוש נושא או מילה';const results=dreamEl('div');
 const show=()=>{results.replaceChildren();const q=dreamNormalize(search.value);const rows=catalog.filter(r=>!q||dreamNormalize(r.name+' '+r.keys.join(' ')).includes(q));results.append(dreamEl('p',rows.length+' תוצאות'+(rows.length>25?' · מוצגות 25 הראשונות, אפשר לצמצם בחיפוש':''),'small'));for(const r of rows.slice(0,25)){const d=dreamEl('details');d.append(dreamEl('summary',r.name),dreamEl('p',r.meaning),dreamEl('p',r.question));results.append(d);}};search.oninput=show;wrap.append(search,results);document.querySelector('.dream-journal').after(wrap);show();
